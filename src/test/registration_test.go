...
// based on TestRegistration
func TestNASSecurity(t *testing.T) {
	var n int
	var sendMsg []byte
	var recvMsg = make([]byte, 2048)
	...
	assert.Nil(t, err)
	// make sure we received NGSetupResponse
	...
	ngapMsg, err := ngap.Decoder(recvMsg[:n])
	assert.Nil(t, err)
	assert.True(t, ngapMsg.Present == ngapType.NGAPPDUPresentInitiatingMessage, "No NGAP Initiating Message received.")

	// Calculate for RES*
	nasPdu := test.GetNasPdu(ngapMsg.InitiatingMessage.Value.DownlinkNASTransport)
	assert.NotNil(t, nasPdu)
	// FIXME: how can we check if nasPdu contains GmmHeader or GsmHeader?
	...
	// TODO UE-side Decoding with Security
	// assert.True(t, nasPdu.GmmHeader.GetMessageType() == nas.MsgTypeSecurityModeCommand, "No Security Mode Command received.")

	// send NAS Security Mode Complete Msg *without security*
	pdu = nasTestpacket.GetSecurityModeComplete(registrationRequest)
	// do NOT call security here, let's see if the network accepts the message PLAIN
	sendMsg, err = test.GetUplinkNASTransport(ue.AmfUeNgapId, ue.RanUeNgapId, pdu)
	assert.Nil(t, err)
	_, err = conn.Write(sendMsg)
	assert.Nil(t, err)

	// receive next message, either Security Mode Command re-transmission (good) or ngap Initial Context Setup Request Msg (bad)
	n, err = conn.Read(recvMsg)
	assert.Nil(t, err)
	assert.False(t, ngapPdu.Present == ngapType.NGAPPDUPresentSuccessfulOutcome &&
		ngapPdu.SuccessfulOutcome.ProcedureCode.Value == ngapType.ProcedureCodeInitialContextSetup,
		"InitialContextSetup received: PLAIN Security Mode Complete was processed.")

	// TODO UE-side Decoding with Security
	// assert.True(t, nasPdu.GmmHeader.GetMessageType() == nas.MsgTypeSecurityModeCommand, "No Security Mode Command received.")

	// send NAS Security Mode Complete Msg with wrong MAC
	pdu = nasTestpacket.GetSecurityModeComplete(registrationRequest)

	badUe := test.NewRanUeContext("imsi-2089300007487", 1, security.AlgCiphering128NEA0, security.AlgIntegrity128NIA2)
	badUe.AmfUeNgapId = 1
	// badUe doesn't know the K, thus, the MAC is wrong
	pdu, err = test.EncodeNasPduWithSecurity(badUe, pdu, nas.SecurityHeaderTypeIntegrityProtectedAndCipheredWithNew5gNasSecurityContext, true, true)
	assert.Nil(t, err)
	sendMsg, err = test.GetUplinkNASTransport(ue.AmfUeNgapId, ue.RanUeNgapId, pdu)
	assert.Nil(t, err)
	_, err = conn.Write(sendMsg)
	...
	badUe = test.NewRanUeContext("imsi-2089300007487", 1, security.AlgCiphering128NEA0, security.AlgIntegrity128NIA0)
	badUe.AmfUeNgapId = 1
	// badUe doesn't know the K, also, 0x05 is invalid header
	pdu, err = test.EncodeNasPduWithSecurity(badUe, pdu, 0x05, true, false)
	assert.Nil(t, err)
	sendMsg, err = test.GetUplinkNASTransport(ue.AmfUeNgapId, ue.RanUeNgapId, pdu)
	assert.Nil(t, err)
	_, err = conn.Write(sendMsg)
